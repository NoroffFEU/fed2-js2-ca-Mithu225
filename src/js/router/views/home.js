...
function init() {
  ...
  async function renderpost(type) {
    ...
    const response = posts.map((item) => {
      return `
          <div class="w-11/12 sm:w-11/12 md:w-11/12 lg:w-1/2 p-5 text-base border-2 border-transparent rounded-md bg-origin-border [background-image:linear-gradient(white,white),linear-gradient(135deg,var(--color-gradient-from),var(--color-gradient-to))] [background-clip:padding-box,border-box]">
